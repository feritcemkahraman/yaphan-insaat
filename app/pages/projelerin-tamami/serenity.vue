<template>
  <div class="h-screen flex overflow-hidden bg-gray-800 text-white relative">
    ...
    <div class="relative w-full h-full">
      ...
      <div class="absolute left-0 bottom-0 w-1/3 p-12 pb-0 bg-gradient-to-t from-black/80 to-transparent">
        <h2 class="text-5xl font-light mb-6">Çengelköy</h2>
        <p class="text-lg leading-relaxed opacity-90 mb-12 mt-6">
          Projenin mimarisi, aile yaşamı için tasarlanmış sakin, sessiz alanların yanı sıra 
          açık ve davetkar karşılama alanlarından olusan bir villanın tamamlayıcı özelliklerini 
          ifade etmektedir.
        </p>
      </div>
      ...
      <div class="absolute bottom-0 right-0 bg-black/60 p-6 z-20">
        <div class="flex items-center gap-4">
          ...
          <button 
            ...
            :class="{ 'opacity-50 cursor-not-allowed': thumbnailStartIndex === 0 }"
          >
          ...
          <!-- Thumbnails -->
          <div class="flex gap-4 overflow-hidden" :style="{ width: isMobile ? 'calc(100vw - 100px)' : '784px' }">
            <div 
              class="flex gap-4 transition-transform duration-300 ease-in-out"
              :style="{ transform: `translateX(-${thumbnailStartIndex * thumbnailOffset}px)` }"
            >
              <div
                ...
                :style="{ width: isMobile ? 'calc((100vw - 120px) / 2)' : '144px' }"
              >
                <img
                  :src="image"
          ...
          <button 
            ...
            :class="{ 'opacity-50 cursor-not-allowed': thumbnailStartIndex >= images.length - visibleThumbnails }"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
            </svg>
...
export default {
  data() {
    return {
      images: [
        '/projeler/devameden/img/1.jpg',
        '/projeler/devameden/img/2.jpg',
        '/projeler/devameden/img/3.jpg',
        '/projeler/devameden/img/4.jpg',
        '/projeler/devameden/img/5.jpg',
        '/projeler/devameden/img/6.jpg',
        '/projeler/devameden/img/7.jpg',
        '/projeler/devameden/img/8.jpg',
      ],
      currentIndex: 0,
  ...
  methods: {
    ...
    nextThumbnail() {
      if (this.thumbnailStartIndex < this.images.length - this.visibleThumbnails) {
        this.thumbnailStartIndex++
      }
    },
    ...
    ensureCurrentImageVisible() {
      ...
      } else if (this.currentIndex >= this.thumbnailStartIndex + this.visibleThumbnails) {
        this.thumbnailStartIndex = Math.max(0, this.currentIndex - (this.visibleThumbnails - 1))
      }
    },
